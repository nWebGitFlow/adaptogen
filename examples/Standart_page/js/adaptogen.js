...
$(document).ready(function(){ 
  ...
  /**
   * ВЫСТАВЛЯЕМ НАСТРОЙКИ СТРАНИЦЫ В СООТВЕТСТВИИ С ПЕРЕМЕННОЙ trackResize В ДАННОМ СКРИПТЕ
   */
  $('#check1').prop('checked', trackResize);
  ...
  function flashpos() { 
    ...
    /**
     * ДОБАВЛЕННЫЙ ВЫЗОВ НОВОЙ ФУНКЦИИ
     */
    checkWinSize(); 
   ...
   * (Добавленная пользовательская функция)
   */
  function checkWinSize() {
    var vps = document.getElementById('zoom');
    s=getVPScale();
    // console.log("checkWinSize.getVPScale = "+s); 
    vps.innerText = s;

    var ps = document.getElementById('page');
    s=getPageScale();
    // console.log("checkWinSize.getPageScale = "+s); 
    ps.innerText = s;

    var in_text  = document.getElementById("instxt");
    s=getMinimumFontSize();
    // console.log("checkWinSize.getMinimumFontSize = "+s); 
    in_text.innerText = getMinimumFontSize();
  };
   ...
   * (Добавленная пользовательская функция)
   * Включение/отключение отслеживания изменения размеров окна браузера
   */
  $('#check1:checkbox').change(function () {
    if($('#check1').prop('checked')) {
      trackResize = true;
      // console.log("Флажок установлен: "+trackResize);
    } else {
      trackResize = false;
      // console.log("Флажок не установлен: "+trackResize);
    };
    window.onresize = (trackResize) ? first_zoom_param : flashpos;
    if (trackResize) first_zoom();
  });
